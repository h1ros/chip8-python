...
class MyChip8(object):
    ...
    def draw(self, Vx, Vy, height):
        self.V[0xF] = 0
        logger.info(
            f'memory from I ({self.I}) to I + height ({self.I + height})')
        ...
        for y in range(0, height):
            ...
            logger.info(
                f"raw {pixel} {bin(pixel)[2:].zfill(8).replace('0', '□').replace('1', '■')} at (x, y) = ({Vx}, {Vy + y})")
            for x in range(0, 8):
    ...
    def emulate_cycle(self):
        ...
        """
        logger.info(
            f'opcode: {hex(self.memory[self.pc])[2:]} {hex(self.memory[self.pc+1])[2:].zfill(2)} ')
        ...
        if self.opcode & 0xFFFF == 0x00EE:
            ...
            logger.info(
                f"# 2NNN: *(0xNNN)() Jump to subroutine at adress NNN ({NNN})")
            self.sp += 1
            ...
            NN = self.opcode & 0x00FF
            logger.info(
                f"# 4XNN: if (Vx ({self.V[x]}) at x ({x}) != NN ({NN}))")
            if self.V[x] != NN:
                logger.info(f"    The condition holds. pc += 2")
                self.pc += 2
        elif self.opcode & 0xF000 == 0x5000:
            logger.info(
                f"#5XY0: if (Vx ({self.V[x]}) at x ({x}) == Vy ( ({self.V[y]}))) at y ({y})")
            ...
            NN = self.opcode & 0x00FF
            logger.info(
                f"#7XNN: Vx ({self.V[x]}) at x ({x}) += NN ({NN}) -> Vx ({self.V[x] + NN})")
            if self.V[x] > 255 - NN:
            ...
            if case == 0:
                ...
                self.V[0xF] = int(bin(self.V[x])[2:].zfill(8)[0])
            ...
            logger.info(
                f"#ANNN: I = NNN (sets I to the adress NNN ({self.opcode & 0x0FFF}))")
            ...
            logger.info(
                f"#DXYN: draw(Vx ({self.V[x]}) at x({x}), Vy ({self.V[y]}) at y ({y}), N height ({N}))")
            self.draw(self.V[x], self.V[y], N)
        elif self.opcode & 0xF0FF == 0xE09E:
            logger.info(
                f"#EX9E: if (key() ({self.keys[self.V[x]]}) ==  1 ~ Vx ({self.V[x]}) at x({x}))")
            if self.keys[self.V[x]] != 0:
                logger.info(f'key is pressed')
                self.pc += 2
        elif self.opcode & 0xF0FF == 0xE0A1:
            logger.info(
                f"#EXA1: if (key() ({self.keys[self.V[x]]}) != 1 ~ Vx ({self.V[x]}) at x({x}))")
            if self.keys[self.V[x]] == 0:
                logger.info(f'key is not pressed')
                self.pc += 2
        elif self.opcode & 0xF0FF == 0xF007:
            logger.info(
                f"#FX07: Vx ({self.V[x]}) at x({x})) = get_delay() ({self.delay_timer})")
            self.V[x] = self.delay_timer
        elif self.opcode & 0xF0FF == 0xF00A:
            logger.info(
                f"#FX0A: Vx ({self.V[x]}) at x({x})) = get_key() ({self.get_key()})")
            ...
            logger.info(
                f"#FX55: reg_dump(Vx ({self.V[x]}) at x ({x}), &I ({self.I}))")
            logger.info(f'V0 ~ VX: {self.V[:x]}')
            for i in range(x + 1):
                self.memory[self.I + i] = self.V[i]
        elif self.opcode & 0xF0FF == 0xF065:
            logger.info(
                f"#FX65: reg_load(Vx ({self.V[x]}) at x ({x}), &I ({self.I}))")
            for i in range(x + 1):
